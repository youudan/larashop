  ...
  <div class="col-md-6 mb-3">
    <form action="{{ route('books.trash') }}">
      <div class="input-group">
        <input class="form-control" value="{{ Request::get('keyword') }}" type="text" name="keyword" placeholder="Title" disabled>
        <div class="input-group-append">
          <button class="btn btn-primary" type="submit" disabled>Filter</button>
        </div>
      </div>
    </form>
  </div>
  ...
  @foreach($books as $book)
    <tr>
      ...
      <td class="text-center align-middle">
        <div class="btn-group">
          <a class="btn btn-success" href="{{ route('books.restore', [$book->id]) }}">Restore</a>
          <form class="d-inline ml-2" action="{{ route('books.delete-permanent', [$book->id]) }}" method="POST" onsubmit="return confirm('Delete permanent this book ?')">
            @csrf
            @method('DELETE')
